...
2つ目は**記号の線の太さが違う天気図**。国立国会図書館由来の2000〜2022年の
天気図がこれで、書体も配置も2023年以降と同じに見えるが、記号の線が細い
(塗りの割合が11%、2023年以降のテンプレートは19〜33%)。**大きさの調整では
直らない** ― 基準の幅を1052〜1900pxのどこに置いても検出はほぼ0個だった。
直すには**その時代の天気図からテンプレートを切り直す**しかない
(`data/templates/README.md` の手順、別フォルダに置いて `--templates` で渡す)。
実測では、記号だけをきれいに切り出せば同じ天気図の他の記号にも当たった
(しきい値0.65で3個)。**周りの等圧線を消し忘れると自分自身にしか当たらない。**
