...
def main():
    ...
    if not args.no_calibration:
        fit_calibration(out_path, eval_base, splits["val"], device, args)


def fit_calibration(weights_path, dataset, val_rows, device, args):
    ...
    校正は学習済みの重みを一切変えない。ロジットを確率に直す部分だけを調整するので、
    しきい値を選び直すかぎり F1 も macro AP も変化しない。
    """
    best_model, meta = load_model(weights_path, map_location=device)
    best_model.to(device).eval()

    logits, targets = calib.collect_logits(
        best_model, dataset, val_rows, device, args.batch_size
    )
